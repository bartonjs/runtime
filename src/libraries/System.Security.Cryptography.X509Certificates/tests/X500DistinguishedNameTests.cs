    ...
    public static class X500DistinguishedNameTests
    {
        ...
        public static void EnumeratorWithInvalidData()
        {
            ...
            foreach (X500RelativeDistinguishedName rdn in dn.EnumerateRelativeDistinguishedNames(reversed: false))
            {
                ...
                {
                    case 0:
                        Assert.False(rdn.HasMultipleValues, $"rdn.HasMultipleValues {index}");
                        Assert.Equal("1.3.6.1.4.1.311.60.2.1.3", rdn.SingleValueType.Value);
                        Assert.Equal("US", rdn.GetSingleValueValue());
                        break;
                    case 1:
                        Assert.False(rdn.HasMultipleValues, $"rdn.HasMultipleValues {index}");
                        Assert.Equal("2.5.4.98", rdn.SingleValueType.Value);
                        Assert.Equal("USA", rdn.GetSingleValueValue());
                        break;
                    case 2:
                        Assert.False(rdn.HasMultipleValues, $"rdn.HasMultipleValues {index}");
                        Assert.Equal("2.5.4.99", rdn.SingleValueType.Value);

                        CryptographicException ex = Assert.Throws<CryptographicException>(
                            () => rdn.GetSingleValueValue());
                    ...
                    case 3:
                        Assert.False(rdn.HasMultipleValues, $"rdn.HasMultipleValues {index}");
                        Assert.Equal("2.5.4.99", rdn.SingleValueType.Value);
                        Assert.Equal("840", rdn.GetSingleValueValue());
                        break;
                    case 4:
                        Assert.False(rdn.HasMultipleValues, $"rdn.HasMultipleValues {index}");
                        Assert.Equal("2.5.4.6", rdn.SingleValueType.Value);
                        Assert.Equal("US", rdn.GetSingleValueValue());
                        break;
                    default:
            ...
            foreach (X500RelativeDistinguishedName rdn in dn.EnumerateRelativeDistinguishedNames(reversed: false))
            {
                switch (index)
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        Assert.True(rdn.HasMultipleValues, $"rdn.HasMultipleValues {index}");
                        Assert.Null(rdn.SingleValueType);
                        Assert.Throws<InvalidOperationException>(() => rdn.GetSingleValueValue());
