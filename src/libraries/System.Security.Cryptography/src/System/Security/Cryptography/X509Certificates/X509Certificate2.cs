        ...
        public bool MatchesHostname(string hostname, bool allowWildcards = true, bool allowCommonName = true)
        {
            ...
            foreach (X509Extension extension in Pal.Extensions)
            {
                if (extension.Oid!.Value == Oids.SubjectAltName)
                {
                    ...
                    {
                        throw new CryptographicException("Multiple Subject Alternative Name extensions present");
                    }
                }
            }
            ...
            if (allowCommonName)
            {
                    ...
                    SubjectName.EnumerateRelativeDistinguishedNames())
                {
                    ...
                    {
                        // Since X500DN already read it with DER, we can use BER here and save on some ifs.
                        AsnValueReader reader = new AsnValueReader(rdn.RawData.Span, AsnEncodingRules.BER);
                        AsnValueReader set = reader.ReadSetOf();
